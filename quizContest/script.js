let quizData = [
  ...
  },
  {
    ...
    questions: [
      ...
      {
        question:
          "Which cricketer has the record of highest individual score in ODIs?",
        options: [
          "Rohit Sharma",
          "Martin Guptill",
          "Chris Gayle",
  ...
  },
  {
    ...
    questions: [
      ...
      {
        question:
          "Which of these is the only metal that is liquid at ordinary room temperature and pressure?",
...
topic.forEach((e) => {
  e.addEventListener("click", selectedTopics);
});
function selectedTopics(e) {
  let element = e.currentTarget.children;
  if (!topics[element[0].innerText]) {
    topics[element[0].innerText] = 1;
    element[0].style.background = "#FCC822";
    element[1].style.display = "flex";
    return;
  }
  delete topics[element[0].innerText];
  element[0].style.background = "grey";
  element[1].style.display = "none";
}

btn2.addEventListener("click", displayTopics);
close1.addEventListener("click", displayTopics);
function displayTopics() {
  getComputedStyle(sec3).display === "none"
    ? (sec3.style.display = "flex")
    : (sec3.style.display = "none");
}
let questions1;
btn3.addEventListener("click", startQuiz);
function startQuiz() {
  let array = Object.keys(topics);
  if (array.length >= 3) {
    ...
    quizData.forEach((item) => {
      if (array.includes(item.category)) {
        array2.push(...item.questions);
      }
    });
    questions1 = array2;
    enteredInto();
  } else {
    alert("Atleast you have to select 3 topics");
  }
}
let index = 0;
let answers = [];
let actualAnswer = [];
function enteredInto() {
  ques.innerText = questions1[index].question;
  for (let i = 0; i < 4; i++) {
    label[i].innerText = questions1[index].options[i];
    input[i].value = questions1[index].options[i];
  }
  actualAnswer[index] = questions1[index].answer;
  if (answers[index]) {
    form.option.value = answers[index];
  }
}
let count = 0;
function next() {
  if (questions1.length - 1 === index) {
    if (count === 0) {
      actualAnswer.forEach((item, i) => {
        item === answers[i] ? count++ : count;
      });
      new Promise((resolve) => {
        setTimeout(() => {
          resolve(buttons[1]);
        ...
        .then((data) => {
          buttons[0].innerText = `Your scored ${actualAnswer.length} out of ${count}`;
  ...
  answers[index] = form.option.value;
  form.reset();
  index++;
  enteredInto();
}
